...
const ControlRoom = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-black text-white overflow-hidden font-mono">
      
      {/* Top Tactical Bar */}
      <div className="h-14 bg-slate-900 border-b border-white/10 flex items-center justify-between px-6 shrink-0 relative z-20">
         <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
               <div className="p-1.5 bg-red-600 rounded text-white"><Radio className="h-4 w-4 animate-pulse" /></div>
               <span className="text-xs font-black tracking-widest uppercase">Tactical Comms Room</span>
            </div>
         ...
         <div className="flex items-center gap-3">
            <button onClick={() => setIsLive(!isLive)} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${isLive ? 'bg-red-600/20 text-red-500 border border-red-500/50' : 'bg-slate-800 text-slate-500'}`}>
               {isLive ? '● Live Transmission' : 'Transmission Paused'}
            </button>
            <div className="bg-slate-800 p-2 rounded-lg cursor-pointer hover:bg-slate-700"><Monitor className="h-4 w-4" /></div>
         </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
         ...
         <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
               ...
               {FEEDS.map(feed => (
                 <div key={feed.id} className="relative aspect-video bg-slate-900 rounded-2xl border border-white/5 overflow-hidden group">
                    ...
                    <div className="absolute top-4 left-4 flex items-center gap-2">
                       <span className={`w-2 h-2 rounded-full ${feed.active ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500'}`}></span>
                       <span className="text-[10px] font-black tracking-widest uppercase text-white shadow-sm">{feed.name}</span>
                    </div>
                    
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                       <div className="space-y-1">
                          <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Coordinates</p>
                          <p className="text-[10px] font-bold">28.6139° N, 77.2090° E</p>
                       </div>
                       <div className="flex gap-2">
                          <button className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white transition-all"><Zap className="h-3 w-3" /></button>
                          <button className="p-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-white transition-all"><ShieldAlert className="h-3 w-3" /></button>
                       </div>
                    </div>

                    {/* Camera Grid Markers */}
                    <div className="absolute top-2 right-2 border-t border-r border-white/20 w-4 h-4"></div>
                    <div className="absolute bottom-2 left-2 border-b border-l border-white/20 w-4 h-4"></div>
                 </div>
               ))}
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
               {[
                 ...
                 <div key={i} className="bg-slate-900/50 border border-white/5 p-4 rounded-xl flex items-center justify-between">
                    <span className="text-[10px] font-black text-slate-500 tracking-widest">{stat.label}</span>
                    <span className={`text-xs font-black text-${stat.color}-500`}>{stat.val}</span>
                 </div>
               ))}
            </div>
         </div>
         ...
         <div className="w-96 bg-slate-900/50 border-l border-white/10 flex flex-col overflow-hidden shrink-0">
            <div className="p-4 border-b border-white/10 bg-slate-900 flex items-center justify-between">
               <h3 className="text-xs font-black uppercase tracking-[0.2em] flex items-center gap-2">
                  <Terminal className="h-4 w-4 text-emerald-500" /> Operations Log
               </h3>
               ...
               <AnimatePresence initial={false}>
                  {logs.map((log) => (
                    ...
                    >
                       ...
                       <p className={`text-[11px] leading-relaxed ${log.status === 'critical' ? 'text-red-400 font-bold' : 'text-slate-300'}`}>
                          {log.text}
                       </p>
                    </motion.div>
                  ))}
            ...
            <div className="p-4 border-t border-white/10 bg-slate-900/80">
               <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Terminal Ready</span>
               </div>
               <div className="relative">
                  <input 
                    type="text" readOnly placeholder="SYSTEM READ ONLY"
                    className="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-[10px] text-emerald-500 font-bold placeholder:text-slate-700 focus:outline-none"
                  />
                  <Loader2 className="absolute right-3 top-2 h-4 w-4 text-slate-700 animate-spin" />
               </div>
            </div>
         </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
         ...
         <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-emerald-600/5 rounded-full blur-[120px]"></div>
      </div>
